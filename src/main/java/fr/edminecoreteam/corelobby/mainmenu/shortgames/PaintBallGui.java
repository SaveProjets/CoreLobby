...
import fr.edminecoreteam.corelobby.stats.sql.PaintBallData;
import fr.edminecoreteam.corelobby.utils.SkullNBT;

public class PaintBallGui implements Listener {
    private static Core api = Core.getInstance();

    public static void gui(Player p) {

        Inventory inv = Bukkit.createInventory(null, 54, "§8Menu Principal (PaintBall)");
        ...
        if (settingInfo.getLang() == 0) {
            ItemStack deco = new ItemBuilder(Material.STAINED_GLASS_PANE, 1, (byte) 4)
                    .setName("§r")
            ...
            ItemStack deco1 = new ItemBuilder(Material.IRON_FENCE, 1)
                    .setName("§r")
                    .toItemStack();
            inv.setItem(37, deco1);
            inv.setItem(38, deco1);
            inv.setItem(39, deco1);
            inv.setItem(40, deco1);
            inv.setItem(41, deco1);
            inv.setItem(42, deco1);
            inv.setItem(43, deco1);
            ...
            loreplay.add(" §f▶ §7Joueur(s):  §e" + OnlinesForGroups.getOnlinesFromGroup("paintball2vs4"));
            loreplay.add(" §f▶ §7File d'attente: ");
            loreplay.add("");
            loreplay.add(" §6Mode de jeu:");
            loreplay.add(" §f➟ §a[2T/4J]");
            loreplay.add(" §f▶ §8[4T/4J]");
            loreplay.add(" §f▶ §8[6T/4J]");
            loreplay.add("");
            ...
            PaintBallData pbData = new PaintBallData(p.getName());

            ArrayList<String> lorestat = new ArrayList<String>();
            lorestat.add("");
            lorestat.add(" §dInformations:");
            lorestat.add(" §f▶ §7Victoires: §a" + pbData.getWin());
            lorestat.add(" §f▶ §7Défaites: §c" + pbData.getLose());
            lorestat.add(" §f▶ §7Parties joués: §e" + pbData.getPlayedGames());
            lorestat.add(" §f▶ §7Kills: §a" + pbData.getKills());
            lorestat.add(" §f▶ §7Morts: §c" + pbData.getDeaths());
            lorestat.add(" §f▶ §7Meilleur Kill-Streak: §d" + pbData.getTopKillStreak());
            lorestat.add("");
